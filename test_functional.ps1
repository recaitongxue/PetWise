...
# Helper function
function Test-ComponentFunctions($name, $path, $requiredPatterns, $buttonPatterns) {
    Write-Host "`nTesting $name..." -ForegroundColor Yellow
    
    if (-not (Test-Path $path)) {
        Write-Host "  [FAIL] File not found: $path" -ForegroundColor Red
        $script:issues += "$name: File not found"
        return
    }
    ...
    if ($missingPatterns.Count -eq 0 -and $missingButtons.Count -eq 0) {
        ...
        $script:issues += "$name: Missing patterns and buttons"
    }
}

# User Components - Test actual functionality
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Components Tested: $($fixed.Count + $issues.Count)" -ForegroundColor Cyan
Write-Host "Passed: $($fixed.Count)" -ForegroundColor Green
