<template>
  <div class="schedule-page">
    ...
    <!-- 添加提醒弹窗 -->
    <el-dialog :title="editingReminderId ? '编辑日程提醒' : '添加日程提醒'" v-model="showAddModal" width="500px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editingReminderId ? updateReminder() : submitReminder()">
          {{ editingReminderId ? '保存' : '添加' }}
        </el-button>
      </div>
    </el-dialog>
  </div>
...
const openEditModal = (reminder) => {
  ...
  form.reminder_type = reminder.reminder_type
  form.title = reminder.title
  form.scheduled_date = reminder.scheduled_date
  form.description = reminder.description || ''
  if (reminder.pet_id) {
    selectedPet.value = reminder.pet_id
  }
...
const updateReminder = async () => {
  ...
  try {
    await petsAPI.deleteReminder(editingReminderId.value)

    const petId = selectedPet.value || displayReminders.value.find(r => r.id === editingReminderId.value)?.pet_id
    if (!petId) {
      ElMessage.error('无法确定宠物')
      return
    }

    const response = await petsAPI.addReminder(petId, {
      ...form,
